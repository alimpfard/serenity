...
void ensure_connection(auto& cache, const URL::URL& url, auto job, Core::ProxyData proxy_data = {})
{
    ...
    if (failed_to_find_a_socket && sockets_for_url.size() < MaxConcurrentConnectionsPerURL) {
        ...
        dbgln_if(REQUESTSERVER_DEBUG, "{} will start a connection for URL {}", pthread_self(), url);
    ...
    start_connection(url, job, sockets_for_url, index, elapsed);
}

void start_connection(URL::URL const& url, auto job, auto& sockets_for_url, size_t index, Duration setup_time)
{
    ...
    if (!connection.socket) {
        dbgln_if(REQUESTSERVER_DEBUG, "{} waiting for connection to be created for URL {}", pthread_self(), url);
        Core::deferred_invoke_if([=, &sockets_for_url] { start_connection(url, job, sockets_for_url, index, setup_time); }, [&connection]() -> bool { return connection.socket; });
        return;
    }
