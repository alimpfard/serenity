...
void EventTarget::set_event_handler_attribute(FlyString const& name, HTML::EventHandler value)
{
    ...
    if (!value.callback.is_null()) {
        ...
        auto* function = JS::ECMAScriptFunctionObject::create(target->script_execution_context()->realm().global_object(), name, program->source_text(), program->body_ptr(), program->parameters(), program->function_length(), nullptr, nullptr, JS::FunctionKind::Normal, false, false);
        VERIFY(function);
