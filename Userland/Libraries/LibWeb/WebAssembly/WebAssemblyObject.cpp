...
JS_DEFINE_NATIVE_FUNCTION(WebAssemblyObject::instantiate)
{
    ...
    auto take_exception_and_reject_if_needed = [&] {
        if (vm.exception()) {
            JS::Value rejection_value = vm.exception()->value();
            vm.clear_exception();
            promise->reject(rejection_value);
            return true;
        }
...
JS_DEFINE_NATIVE_GETTER(WebAssemblyModuleInstanceObject::exports_getter)
{
    auto this_value = vm.this_value(global_object);
    if (vm.exception())
        return {};
    auto this_object = this_value.to_object(global_object);
    if (!is<WebAssemblyModuleInstanceObject>(this_object)) {
        vm.throw_exception<JS::TypeError>(global_object, JS::ErrorType::NotAn, "WebAssemblyModuleInstance");
        return {};
    }
    auto object = static_cast<WebAssemblyModuleInstanceObject*>(this_object);
